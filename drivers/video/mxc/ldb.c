...
static struct fb_videomode ldb_modedb[] = {
	{	
	"LDB-WXGA", 60, 1368, 768, 13085, // Evan. 1366x768 Panel CHIMEI 
         180, 0,
         21, 7,
         45, 7,  //hsync_len , vsync_len
         0,
         FB_VMODE_NONINTERLACED,
         FB_MODE_IS_DETAILED,},
	{
	 "LDB-WXGA2", 60, 1368, 768, 13255,
	 100, 40,
	 21, 7,
	 60, 10,
	 0,
	...
	struct mxc_dispdrv_setting *setting)
{
	...
	if (!ldb->inited) {
		...
		if (ldb->mode == LDB_SPL_DI0) {
			...
			{
				lvds_channel = 0;
				printk("first lvds channel is %d\n",lvds_channel);
			}
			else
			{
				lvds_channel = 1;
		...
		if (ldb->mode == LDB_SEP0)
		{	
			printk("Evan second ouput is LVDS1");
			lvds_channel = 1;
		}
		else
		{	
			lvds_channel = 0;
			 printk("Evan second ouput is LVDS0");
		}
		reg = readl(ldb->control_reg);
		if ((lvds_channel == 0) && (setting->disp_id == 0))
			reg |= LDB_CH0_MODE_EN_TO_DI0;
		...
		{
			reg |= LDB_CH1_MODE_EN_TO_DI0;
			 printk("Evan second ouput DISPLAY_ID = 0");

		}
		else
		{
			 printk("Evan second ouput DISPLAY_ID = 1");
			reg |= LDB_CH1_MODE_EN_TO_DI1;
		}
		ch_mask = lvds_channel ?  LDB_CH1_MODE_MASK :
				LDB_CH0_MODE_MASK;
	...
	for (i = 0; i < ldb_modedb_sz; i++) {
		...
		if (fb_mode_is_equal(&m, &ldb_modedb[i])) {
			...
			printk("Evan find ldb mode target %s\n", ldb_modedb[i].name);
			printk("find device name %s\n ",m.name);				
			break;
		}
	}
