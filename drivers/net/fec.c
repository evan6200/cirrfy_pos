...
static int fec_enet_mii_probe(struct net_device *ndev)
{
	...
	for (phy_id = 0; (phy_id < PHY_MAX_ADDR); phy_id++) {
		printk("Evan fec.c PHY_MAX_ADDR = %d \n" , PHY_MAX_ADDR);
	...
	printk("Evan fec.c mdio bus id = %s \n",mdio_bus_id);
	...
	if (IS_ERR(phy_dev)) {
		printk(KERN_ERR "%s: Evan fec.c could not attach to PHY\n", ndev->name);
		printk(KERN_ERR "Evan fec.c print phy_name = %s" , phy_name);
