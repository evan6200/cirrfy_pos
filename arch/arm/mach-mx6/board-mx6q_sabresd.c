...
#define CIRRFY_SUSPEND_RESUME   IMX_GPIO_NR(1, 27)
...
#define SABRESD_PMIC_INT_B	IMX_GPIO_NR(1, 8)
#define SABRESD_PFUZE_INT	IMX_GPIO_NR(1, 8)
...
#define RTC_IRQ                 IMX_GPIO_NR(5, 8)
...
{
	printk ("Ready to Reset RGMII \n");
        if (gpio_request(SABRESD_MICROPHONE_DET, "rgmii_rst"))
	{
                pr_err("Evan fail to Reset RGMII\n");
                return;
        }
        else {
                printk("Evan Reset RGMII , SABRESD_MICROPHONE_DET \n");      
                gpio_direction_output(SABRESD_MICROPHONE_DET, 0);
		udelay(40);
        	gpio_direction_output(SABRESD_MICROPHONE_DET, 1);
	        gpio_export(SABRESD_MICROPHONE_DET, 1);
        }
...
static void cheme_lcd_init(void)
{
	...
	unsigned int reg;
	printk("Ready to enable LCD");

	udelay(20);
        if (gpio_request(CHEME_LCD_EN, "cheme_lcd_en")){
                pr_err("Evan failed to get GPIO CHEME_LCD_EN\n");
                return;
        }
        else {
		printk("Evan enable gpio 7, 13 ,CHIME LCD EN \n");	
	 ...
	 udelay(20);
        if (gpio_request(CHEME_LCD_BLED_EN, "cheme_lcd_bled_en")){
                printk("Evan failed to get GPIO CHEME_LCD_BLED_EN\n");
                return;
        }
        else {	
...
/*
	         udelay(20);
        if (gpio_request( EVAN_POWER_LED, "power_led")){
                printk("Evan failed to get GPIO power_led \n");
                return;
        }
        else {
                printk("Evan Enable gpio 2,4");
                gpio_direction_output( EVAN_POWER_LED, 1);
                gpio_export( EVAN_POWER_LED, 0);
        }
...
#define CIRRFY_SD2_CD           IMX_GPIO_NR(1,4) //Evan modified 2014.1.2
static const struct esdhc_platform_data mx6q_sabresd_sd2_data __initconst = {
//        .cd_gpio = SABRESD_SD2_CD,
        .cd_gpio = CIRRFY_SD2_CD, //Evan modofied for cirrfy pos
//        .wp_gpio = SABRESD_SD2_WP,
        .wp_gpio =  -EINVAL ,
        .keep_power_at_suspend = 1,
        .support_8bit = 1,
//      .support_8bit = 0, // 4 bit
        .delay_line = 0,
        .cd_type = ESDHC_CD_CONTROLLER,
};
...
//Evan modifeid for comport 3 fail.
...
static struct imx_ssi_platform_data mx6_jht_ssi_pdata = {
	.flags = IMX_SSI_DMA | IMX_SSI_SYN,
};
...
static struct i2c_board_info mxc_i2c0_board_info[] __initdata = {
	{
                I2C_BOARD_INFO("tlv320aic3x", 0x18), //Evan modified
        },
...
//Evan modified
	mxc_register_device(&mx6_audio_tlv320aic3x_device,
				    &tlv320aic3x_data);
	imx6q_add_imx_ssi(1, &mx6_jht_ssi_pdata);
...
static struct gpio_keys_button imx6q_buttons[] = {
	...
	GPIO_BUTTON(CIRRFY_SUSPEND_RESUME, KEY_POWER, 1, "power", 1, 1),
};
