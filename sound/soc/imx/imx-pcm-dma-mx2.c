				...
				struct snd_pcm_hw_params *params)
{
	...
	//printk("Evan in : snd_imx_pcm_hw_params\n");
	dma_params = snd_soc_dai_get_dma_data(rtd->cpu_dai, substream);

	if (iprtd->asrc_enable) {
	//	printk("Evan in :  iprtd->asrc_enable)=  true\n");
		ret = imx_ssi_asrc_dma_alloc(substream, params);
		if (ret)
		{	
	//		printk("Evan in : snd_imx_pcm_hw_params , imx_ssi_asrc_dma_alloc return ret =   %d \n ", ret);
			return ret;
		}
	...
	//			printk("Evan in : snd_imx_pcm_hw_params ,cannot prepare slave dma\n ");
			return -EINVAL;
		}
		...
		if (ret)
			{
	//			printk("Evan in : snd_imx_pcm_hw_params , imx_ssi_dma_alloc , ret = %d\n " ,ret);
				return ret;
			}
		chan = iprtd->dma_chan;
	}
	...
	if (iprtd->asrc_enable) {
		...
		if (!iprtd->asrc_desc) {
			dev_err(&chan->dev->device,
					"cannot prepare slave dma\n");
	//				printk("Evan in : snd_imx_pcm_hw_params , cannot prepare slave dma - 1 \n ");
			return -EINVAL;
		}
		...
		if (!iprtd->desc) {
			dev_err(&chan->dev->device,
					"cannot prepare slave dma\n");
	//				printk("Evan in : snd_imx_pcm_hw_params , cannot prepare slave dma - 2 \n ");
			return -EINVAL;
		}
//printk("Evan in : snd_imx_pcm_hw_params , iprtd->desc->callback = audio_dma_irq;  \n ");
...
static int snd_imx_open(struct snd_pcm_substream *substream)
{
	...
	int ret;
//	printk("Evan in snd_imx_open \n");
	iprtd = kzalloc(sizeof(*iprtd), GFP_KERNEL);
	if (iprtd == NULL)
	{
//		printk("Evan in snd_imx_open iprtd == NULL \n");
		return -ENOMEM;
	}
	if (!strcmp(rtd->dai_link->name, "HiFi_ASRC")) {
		...
		iprtd->asrc_index = -1;
		if (!asrc_pcm_p2p_ops) {
//			printk("Evan in snd_imx_open : ASRC is not loaded!\n");
	...
	if (ret < 0) {
//		printk("Evan in snd_imx_open : ret < 0 lfree(iprtd)  ret = %d\n "  , ret);
		kfree(iprtd);
		return ret;
	}
